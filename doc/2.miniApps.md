...
This guide describes how to create a miniApp.
...
Enable Devel_Mode only on a trusted network, and turn it off when you are done.
...
This section describes how to setup an Ubuntu PC to develop miniApps. This has
been tested using Ubuntu 26.04.

A bash script is provided to do the setup.
Download and run the bash script, and follow the instructions provided in the script.
```
cd ~
...
Windows Subsystem for Linux (WSL) will be used. Refer to "Appendix - Using WSL".
...
A bash script is provided to do the setup.
Download and run the bash script, and follow the instruction provided in the script.
```
cd ~
wget https://raw.githubusercontent.com/sthaid/ezApp/refs/heads/main/bin/setup_devel_pc
chmod +x setup_devel_pc
...
- On Devel PC, env var EZAPP_DEVICE must be set to Android device IP address
- On Devel PC, env var EZAPP_PASSWD must be set to the Devel_Password
- These env vars are set by setup_devel_pc in ~/.bashrc_ezApp; confirm them
...
Create a new miniApp, starting with a copy of the Example1 miniApp.
```
cd ~/ezApp/files/apps
cp -r Example1 MyNewApp
cd MyNewApp
vi example1.c    # change "Hello\nWorld" to "MyNewApp"
```
...
cd ~/ezApp/linux
make run
locate MyNewApp on the menu pages, and tap to run
```

Run the new miniApp on Android
==============================

Ensure ezApp is running on the Android device; and that ezApp Devel_Mode is ON.

In a new terminal session, on the Devel PC, run ```ezsh logwatch```
to view ezApp debug print messages from ezApp and your MyNewApp.

Copy the new miniApp to the Android Device.
```
cd ~/ezApp/files/apps/MyNewApp
ezput
```

MyNewApp should automatically appear on the Android Device ezApp menu.
Tap '>' to page through the menu to locate MyNewApp.

Tap MyNewApp to run it.

APIs available for use by miniApps
==================================

The PicoC C language interpreter is extended to support the APIs defined
in the ~/ezApp/src/ezApp_lib/include files::
...
- profile.h: gathers performance info, shows the most often executed lines of code
...
Shared miniApp helpers are in files/lib/lib.h (see files/lib/README).
...
* Macros must not be defined within a procedure, and macros must expand to an expression.
...
MiniApps must free all memory allocations and close all files, when exiting,
...
- if a file named 'stopped' exists in the miniSvc directory, that miniSvc is not automatically started
...
Refer to svcs.h for API details; and view example in the Example miniSvc.

Only the following APIs can be used by a miniSvc.
- sdlx.h: sensors, sdlx_show_toast
- svcs.h: svc_wait_for_req, svc_req_completed
...
```
cd ~/ezApp/files/svcs
cp -r Example MyNewSvc
cd MyNewSvc/
ezput
```
- On ezApp: Settings > Services; tap MyNewSvc to start the new miniSvc.
...
wsl --status                      # should indicate Default Version: 2
